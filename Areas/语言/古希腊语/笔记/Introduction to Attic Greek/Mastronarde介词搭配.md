---
title: "专题整理 - 介词搭配"
created: 2026-09-13
language: "古希腊语"
tags: [language/greek]
---
[[Mastronarde Greek - D. J. Mastronarde|返回导航]] | [[主页|返回主页]] | [[Introduction to Attic Greek (2nd edition) (Donald J. Mastronarde) (z-library.sk, 1lib.sk, z-lib.sk).pdf|教材链接]]

---

# 介词搭配（Prepositions + Case）

希腊语介词支配**属格 / 与格 / 宾格**三个斜格之一或多个。同一介词带不同格时，意义往往不同。本页汇总 Mastronarde 全书出现的介词及其搭配，并在每个多格介词词条下直接附助记。

## 总原则：格决定方向

介词的格与方位语义大体对应（Mastronarde Unit 3.7、Unit 8.3）：

| 格 | 语义倾向 |
| --- | --- |
| gen. | 表示**离开**（motion away from）—— 来源、分离、施事 |
| dat. | 表示**静止位置**（static position）—— 处所、工具、伴随 |
| acc. | 表示**朝向**（motion toward）—— 方向、终点、范围 |

**为什么是这样？** 这三格继承自**原始印欧语**的方位格系统：原始印欧语有夺格（ablative，表「从……」）、方位格（locative，表「在……」）、工具格（instrumental，表「用……」）；希腊语把**夺格并入属格**、**方位格 + 工具格并入与格**，宾格则保留「方向 / 终点」。因此属格天然带「来源」义，与格天然带「处所 / 工具」义。

> **记忆钥匙**：拉丁语保留了独立夺格，所以拉丁语表「从……」用夺格，希腊语里就换成属格 —— 见「希腊介词 + gen.」时，先想拉丁 **ab / de / ex / pro + abl.**。

**一句话总纲**：**gen. 问「从哪来」→ dat. 问「在哪」→ acc. 问「到哪去」**。多格介词就是同一个词在这三问之间取义；不确定格时先代入三问，再用拉丁语对号（ab/de/ex/pro → gen.；in/sub/cum/apud → dat.；ad/in/per/secundum/post/contra → acc.）。

---

## 可接格位总览

每一行一个介词，标 √ 者表示可支配该格。

| 介词 | gen. | dat. | acc. | 格数 |
| --- | :---: | :---: | :---: | :---: |
| ἀπό | √ |  |  | 单 |
| ἐκ / ἐξ | √ |  |  | 单 |
| ἀντί | √ |  |  | 单 |
| πρό | √ |  |  | 单 |
| ἐν |  | √ |  | 单 |
| σύν |  | √ |  | 单 |
| εἰς / ἐς |  |  | √ | 单 |
| ἀνά |  |  | √ | 单 |
| διά | √ |  | √ | 双 |
| μετά | √ |  | √ | 双 |
| κατά | √ |  | √ | 双 |
| ὑπέρ | √ |  | √ | 双 |
| ἐπί | √ | √ | √ | 三 |
| παρά | √ | √ | √ | 三 |
| πρός | √ | √ | √ | 三 |
| περί | √ | √ | √ | 三 |
| ὑπό | √ | √ | √ | 三 |

**读表要点**：

- 单格介词中，**属格最多（4 个）**，与格、宾格各 2 个 —— 越"专门"的介词越只配一个格。
- 双格介词**全部只出现在 gen. 与 acc.**，没有一个双格介词用 dat.（与格是"位置"格，语义最窄）。
- 三格介词共 5 个：ἐπί, παρά, πρός, περί, ὑπό —— 只有语义最"立体"（可表来源 / 位置 / 方向三者）的介词才配三格。

---

## 单格介词

| 介词 | 支配格 | 释义 | 出处 |
| --- | --- | --- | --- |
| ἀπό（ἀπ'/ἀφ' 元音前） | gen. | away from, from | [[Mastronarde Greek - Unit 3 笔记\|Unit 3]] |
| ἐκ（ἐξ 元音前） | gen. | out of, forth from | [[Mastronarde Greek - Unit 3 笔记\|Unit 3]] |
| ἐν | dat. | in, within, on, at, among | [[Mastronarde Greek - Unit 3 笔记\|Unit 3]] |
| εἰς / ἐς | acc. | into, to, toward | [[Mastronarde Greek - Unit 3 笔记\|Unit 3]] |
| σύν | dat. | together with, with | [[Mastronarde Greek - Unit 3 笔记\|Unit 3]] |
| ἀντί | gen. | instead of, in place of, in return for | [[Mastronarde Greek - Unit 6 笔记\|Unit 6]] |
| πρό | gen. | in front of; in defense of; before | [[Mastronarde Greek - Unit 6 笔记\|Unit 6]] |
| ἀνά | acc. | up, up along, throughout | [[Mastronarde Greek - Unit 12 笔记\|Unit 12]] |

**助记**：属格 4 个（ἀπό, ἐκ, ἀντί, πρό）都是"**从……**"（离开 / 出自 / 代替 / 之前）；与格 2 个（ἐν, σύν）都是"**在某处 / 与某人**"；宾格 2 个（εἰς, ἀνά）都是"**向某处 / 向上**"。

> ἐκ、ἐν、εἰς 是**前附词**（proclitic），通常不重读（Unit 2.11）。

---

## 双格介词

### διά ｜ [[Mastronarde Greek - Unit 6 笔记|Unit 6]]

| 格 | 释义 | 拉丁类比（近似） |
| --- | --- | --- |
| gen. | through（空间 / 时间）；by（施事 / 工具） | per + acc. |
| acc. | because of, on account of, by aid of, by reason of | propter / ob + acc. |

**助记**：核心义「穿过」—— gen. 是穿过某空间 / 某时段 / 借由某工具；acc. 由「经由某原因」抽象为「因为」。**gen. 偏「经由」，acc. 偏「缘由」**。

### μετά ｜ [[Mastronarde Greek - Unit 6 笔记|Unit 6]]

| 格 | 释义 | 拉丁类比（近似） |
| --- | --- | --- |
| gen. | among, with | cum + abl. |
| acc. | after（时间或等级） | post + acc. |

**助记**：核心义「在……中间（amidst）」—— gen. 是身处其中 →「与……一起」；acc. 是次序上「紧随其后」→「在……之后」。**in the midst of → with；next in order → after**（一为空间同时，一为时间先后）。

### κατά ｜ [[Mastronarde Greek - Unit 12 笔记|Unit 12]]

| 格 | 释义 | 拉丁类比（近似） |
| --- | --- | --- |
| gen. | down from, down upon; against | de + abl. / contra + acc. |
| acc. | down along, over, throughout; in accordance with; concerning | secundum + acc. |

**助记**：核心义「向下」—— gen. 是向下**离开**某物（→ 引申「针对、反对」）；acc. 是沿着某物**向下铺开**（→「遍及、按照」）。**gen. 是"离开"（落到某物上 / 冲着某物去），acc. 是"铺开"（沿着、按照）**。

### ὑπέρ ｜ [[Mastronarde Greek - Unit 12 笔记|Unit 12]]

| 格 | 释义 | 拉丁类比（近似） |
| --- | --- | --- |
| gen. | over, above; in defense of; concerning | pro + abl. |
| acc. | over, across, beyond（运动或位置） | super / trans + acc. |

**助记**：核心义「在上方」—— gen. 从上方笼罩 →「为……出头、代表」；acc. 越过某个限度 →「越过、超出」。**gen. 对应 pro（代表），acc. 对应 super（越过）**。

---

## 三格介词

三格介词套用统一的「**接触点 / 停驻点 / 落点**」三联式：**gen. 贴着 → dat. 待着 → acc. 扑上去**；再对照拉丁 **ab + abl.（来源）、in/sub + abl.（位置）、in/sub + acc.（方向）**。

### ἐπί ｜ [[Mastronarde Greek - Unit 8 笔记|Unit 8]]

| 格 | 释义 | 拉丁类比（近似） |
| --- | --- | --- |
| gen. | upon; in the time of | super + abl.（贴附 / 当时） |
| dat. | upon, over; next to; in addition to | in + abl. |
| acc. | onto, up to, toward; against | in + acc. |

**助记**：核心义「在上」—— gen. = 接触点（贴在表面之上 / 在某人的任期之内）；dat. = 停驻点（就在其上 / 其旁 / 附加于）；acc. = 落点（落到其上 / 冲着而去）。**gen. 贴着，dat. 待着，acc. 扑上去**。

### παρά ｜ [[Mastronarde Greek - Unit 8 笔记|Unit 8]]

| 格 | 释义 | 拉丁类比（近似） |
| --- | --- | --- |
| gen. | from the side of, from（宾语多为人） | a / ab + abl. |
| dat. | by the side of, at the house of（宾语几乎必为人） | apud |
| acc. | to the side of; beside, alongside; past, beyond, contrary to | ad / praeter + acc. |

**助记**：核心义「在……旁边」（一条平行线）—— gen. 从旁边来；dat. 在某人处；acc. 到旁边去 / 越过去。**παρὰ + dat. = apud**（最好用的一条）；三点连成一线：从哪来、在哪、去哪。

### πρός ｜ [[Mastronarde Greek - Unit 8 笔记|Unit 8]]

| 格 | 释义 | 拉丁类比（近似） |
| --- | --- | --- |
| gen. | from, proceeding from（宾语多为人） | a / ab + abl. |
| dat. | near, beside; in addition to | apud（近旁） |
| acc. | to, toward; against; in respect to, regarding | ad + acc. |

**助记**：核心义「朝向」—— acc. 最核心：朝过去（to, toward, against）；dat. 就在旁边（near, beside）；gen. 是「从对方那一方来的」（如 πρὸς τῶν πολιτῶν = 从公民那里）。**πρός = "面向"，故 acc. 最核心；gen. 是"从对面来"**。

### περί ｜ [[Mastronarde Greek - Unit 12 笔记|Unit 12]]

| 格 | 释义 | 拉丁类比（近似） |
| --- | --- | --- |
| gen. | about, concerning;（与表「优越」的词连用）above, beyond | de + abl. |
| dat. | around（位置）；about（所争夺的对象） | circum（静态） |
| acc. | around（运动）；about, concerning | circum + acc. |

**助记**：核心义「环绕」—— gen. 环绕某**话题** →「关于」；dat. 环绕某**位置**（静止）；acc. **绕行**某物（运动）。**三大义都从"环绕"长出来：绕话题 = 关于，绕位置 = 在……周围，绕一圈 = 环绕运动**。

### ὑπό ｜ [[Mastronarde Greek - Unit 12 笔记|Unit 12]]

| 格 | 释义 | 拉丁类比（近似） |
| --- | --- | --- |
| gen. | from under, under; by（被动式的施事者） | ab + abl.（施事） |
| dat. | under; under the power of | sub + abl. |
| acc. | under; during, in the course of | sub + acc. |

**助记**：核心义「在下」—— gen. 从下方把动作**推上来** →「被（施事者）」；dat. **被压在下面**（隶属、支配）；acc. **钻到下面去**（运动）/ 在某过程之下（"在……期间"）。**被动句施事者用 ὑπό + gen. ≈ 拉丁 ab + abl.**；"under the power of" 是静压，"during" 是过程覆盖。

---

# 形近词 / 近义词辨析

## 形近字对

### ὑπό vs ὑπέρ（同为 ὑπ-，义相反：下 / 上）

| 格 | ὑπό（下） | ὑπέρ（上） |
| --- | --- | --- |
| gen. | 被（施事者） | 为……（代表） |
| dat. | 在……之下（受支配） | 无 |
| acc. | 到……之下；在……期间 | 越出（超出） |

- 拉丁对号：**ὑπό + gen. ≈ ab + abl.**（施事）；**ὑπέρ + gen. ≈ pro + abl.**（代表）。
- 记：**sub 在下，super 在上**；拉丁 **sub + abl.（位置）/ sub + acc.（方向）** 正是 ὑπό 与格 / 宾格的分工。

### πρό vs πρός（同为 πρ-，义别：在前 / 朝向）

| | πρό | πρός |
| --- | --- | --- |
| 格 | 只 + gen. | 三格 |
| 核心义 | in front of; before（位置 / 时间在前，静止） | to, toward; against（朝向，有方向） |
| 拉丁 | pro + abl. | ad + acc. |

- 区别：**πρό 是"点在前"**（静态，只配属格）；**πρός 是"对着去"**（动态方向，配三格）。
- 字形：πρό（-ο）vs πρός（多一个 -ς）。

### ἐν vs εἰς（同源，静止 / 运动对立）

- 同出印欧 \*en：**ἐν + dat.** = 在……内（静止）；**εἰς / ἐς + acc.** = 进入……内（运动）。
- 对比拉丁：ἐν ≈ in + abl.，εἰς ≈ in + acc.。
- 记：**ἐν 在内不动，εἰς（带 -ς）钻进去**。

## 近义词（按语义场）

### "从……"：ἐκ / ἀπό / παρά（+ gen.）

| 介词 | 出发点 | 拉丁 |
| --- | --- | --- |
| ἐκ / ἐξ | 从**内部**出来 | ex + abl. |
| ἀπό | 从**表面 / 旁边**离开 | ab + abl. |
| παρά | 从**某人身边**（宾语多为人） | a / ab + abl. |

记：**ἐκ 里 → ἀπό 面 → παρά 人**。

### "到……"：εἰς / ἐπί / πρός（+ acc.）

| 介词 | 落点 | 拉丁 |
| --- | --- | --- |
| εἰς / ἐς | 进入**内部** | in + acc. |
| ἐπί | 落到**上面** | in + acc. |
| πρός | 朝向（重方向，不重终点） | ad + acc. |

记：**εἰς 里 → ἐπί 上 → πρός 方向**。（这也是 ἐπί、πρός + acc. 常与 εἰς 意义重叠的原因。）

### "与……一起"：σύν vs μετά（+ gen.）

| | σύν | μετά |
| --- | --- | --- |
| 格 | 只 + dat. | + gen. / + acc. |
| 义 | together with（共同协作） | among, with（在……之中 / 伴随） |
| 拉丁 | cum + abl. | cum / inter |

记：**σύν 强调"一起做"（协作），μετά + gen. 强调"在……中间"（伴随）**。

### 施事者 vs 工具：ὑπό vs διά（+ gen.）

- **ὑπό + gen.** = 施事者，宾语多为**人**（"by X"，≈ ab + abl.）。
- **διά + gen.** = 工具 / 中介，多为**物或手段**（"by means of"）。
- 记：**人被 ὑπό，物用 διά**。

---

# 来源

- [[Mastronarde Greek - Unit 3 笔记]] — Prepositions I（ἀπό, ἐκ/ἐξ, ἐν, εἰς/ἐς, σύν）
- [[Mastronarde Greek - Unit 6 笔记]] — Prepositions II（ἀντί, πρό, διά, μετά）
- [[Mastronarde Greek - Unit 8 笔记]] — Prepositions III（ἐπί, παρά, πρός）
- [[Mastronarde Greek - Unit 12 笔记]] — Prepositions IV（ἀνά, κατά, περί, ὑπέρ, ὑπό）
- 教材：[[Introduction to Attic Greek (2nd edition) (Donald J. Mastronarde) (z-library.sk, 1lib.sk, z-lib.sk).pdf|Introduction to Attic Greek, 2nd ed.]]
